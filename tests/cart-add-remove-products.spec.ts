...
test.describe('Shopping Cart - Add and Remove Products', () => {
  ...
  test('should update product quantity in cart using plus and minus buttons', async ({ page }) => {
    // Step 1: Navigate to the application
    await page.goto('http://localhost:5137');
    await expect(page).toHaveTitle(/OctoCAT Supplies/);

    // Step 2: Navigate to Products page and add a product
    await page.getByRole('link', { name: 'Products' }).click();
    await expect(page).toHaveURL(/\/products/);

    // Add 1 CatFlix Entertainment Portal to cart
    await page.getByRole('button', { name: 'Increase quantity of CatFlix Entertainment Portal' }).click();
    await page.getByRole('button', { name: 'Add 1 CatFlix Entertainment Portal to cart' }).click();
    await expect(page.getByRole('link', { name: /Shopping cart with 1 item/ })).toBeVisible();
    ...
    // Step 4: Increase quantity using plus button in cart
    await page.getByRole('button', { name: 'Increase quantity of CatFlix Entertainment Portal' }).click();
    ...
    // Step 5: Increase quantity again
    await page.getByRole('button', { name: 'Increase quantity of CatFlix Entertainment Portal' }).click();
    
    // Verify quantity updated to 3
    await expect(page.getByRole('link', { name: /Shopping cart with 3 items/ })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Shopping Cart(3 items)' })).toBeVisible();
    await expect(page.getByLabel('Quantity of CatFlix Entertainment Portal', { exact: true })).toHaveText('3');
    await expect(page.getByText('$269.97')).toBeVisible(); // 3 x $89.99

    // Step 6: Decrease quantity using minus button
    await page.getByRole('button', { name: 'Decrease quantity of CatFlix Entertainment Portal' }).click();
    
    // Verify quantity decreased to 2
    await expect(page.getByRole('link', { name: /Shopping cart with 2 items/ })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Shopping Cart(2 items)' })).toBeVisible();
    await expect(page.getByLabel('Quantity of CatFlix Entertainment Portal', { exact: true })).toHaveText('2');
    await expect(page.getByText('$179.98')).toBeVisible();

    // Step 7: Decrease quantity to 1
    await page.getByRole('button', { name: 'Decrease quantity of CatFlix Entertainment Portal' }).click();
    ...
    // Step 8: Decrease quantity to 0 (should remove item from cart)
    await page.getByRole('button', { name: 'Decrease quantity of CatFlix Entertainment Portal' }).click();
